...
class JsonPage extends StatefulWidget {
  const JsonPage({super.key, required this.title});
  final String title;
...
class _JsonPageState extends State<JsonPage> {
  ...
  /// convertToNestedMap
  /// @description JSON 중접 구조로 변환
  Map<dynamic, dynamic> convertToNestedMap(Map<String, dynamic> json) {
    Map<dynamic, dynamic> result = {};

    json.forEach((key, value) {
      final List<String> nestedKeys = key.split('.');
      Map<dynamic, dynamic> temp = result;

      for (int i = 0; i < nestedKeys.length; i++) {
        final currentKey = nestedKeys[i];
  ...
  onChangeSheetName(index) {
    ...
    // print(json.encode(convertToNestedMap(jsonData)));

    setState(() {
      // processedHTML = json.encode(convertToNestedMap(jsonData));
      processedCode = const JsonEncoder.withIndent('  ').convert(convertToNestedMap(jsonData));
    });
